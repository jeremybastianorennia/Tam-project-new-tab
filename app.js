...
function attachEventListeners() {
    try {
        ...
        // Dashboard event listeners - with null checks
...
function clearAllFilters() {
    try {
        ...
        if (minEmployeesInput) minEmployeesInput.value = '';
        if (maxEmployeesInput) maxEmployeesInput.value = '';
...
function highlightSearchTerms() {
    const searchTerm = searchInput?.value?.trim().toLowerCase() || '';
    if (!searchTerm || !resultsBody) return;
